...
using Core;
using Core.Registries.Collections.Pooled;
using Core.Registries.Entities;
using Core.Serializer;
using Core.Serializer.Entities.Accessors;

namespace Tests.Registries.Collections.Pooled;
...
public class PooledDictionaryTest
{
    ...
    {
        var dict = new PooledDictionary<string, string>(StringComparer.Ordinal)
        {
            { "First", "Second" },
            { "Third", "Fourth" }
        };
        ...
        var dictV1 = serializerRegistry.Deserialize<PooledDictionary<string, string>>(readerStream)!;

        var accessor = MemberAccessor.Create(typeof(PooledDictionary<string, string>));
        T Get<T>(PooledDictionary<string, string> dictionary, string name) => (T) accessor[dictionary, name];
