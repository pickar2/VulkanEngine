...
public class ShadercOptions : IDisposable, ICloneable
{
	...
	/// </summary>
	public readonly List<string> IncludeDirectories = new();
	...
	{
		set => NativeMethods.shaderc_compile_options_set_source_language(handle, value);
	}
	...
	{
		set => NativeMethods.shaderc_compile_options_set_optimization_level(handle, value);
	}
	...
	public bool InvertY
	{
		set => NativeMethods.shaderc_compile_options_set_invert_y(handle, value);
	}
	...
	/// </summary>
	public void EnableWarningsAsErrors() => NativeMethods.shaderc_compile_options_set_warnings_as_errors(handle);

	/// <summary>
	///     Sets a resource limit.
	/// </summary>
	public void SetLimit(Limit limit, int value) => NativeMethods.shaderc_compile_options_set_limit(handle, limit, value);
	...
	/// </summary>
	public void SetBindingBase(UniformKind kind, uint _base) => NativeMethods.shaderc_compile_options_set_binding_base(handle, kind, _base);
	...
	public void SetHlslRegisterSetAndBinding(ShaderKind shaderKind, string reg, string set, string binding) =>
		NativeMethods.shaderc_compile_options_set_hlsl_register_set_and_binding_for_stage(handle, shaderKind, reg, set,
			binding);
	...
	private static IntPtr HandlePFN_IncludeResolve(IntPtr userData, string requestedSource, int type,
		string requestingSource, UIntPtr includeDepth)
	{
		var opts = optionsDic[userData.ToInt32()];
	...
	private void setIncludeCallbacks() =>
		NativeMethods.shaderc_compile_options_set_include_callbacks(handle,
			Marshal.GetFunctionPointerForDelegate(resolve),
