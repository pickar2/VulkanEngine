...
void transform_material(FragmentData fragData, MaterialData matData) {
	transform_material_struct mat = transform_material_data[matData.vertexDataIndex];
	fragTexCoord = vertexPos[gl_VertexIndex & 3];

	vec4 fullScreenCoord = globalMatrix * modelMatrix * mat.transformMatrix * vec4(fragData.fragCoord, 1.0, 1.0);
