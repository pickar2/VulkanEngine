...
void cool_material(FragmentData fragData, MaterialData matData) {
	cool_material_struct mat = cool_material_data[matData.fragmentDataIndex];
	vec2 p = vec2(0.001, 0.002);
	vec4 a = vec4(0);

	vec2 coords = fragData.fragCoord.xy;
