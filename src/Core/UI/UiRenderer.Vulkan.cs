...
public static unsafe partial class UiRenderer
{
	...
	public static void Init()
	{
		...
		_sampler = CreateImageSampler(16); // Do we need to filter ui?
		...
		{
			var pool = CreateCommandPool(Context.GraphicsQueue, 0);
			_commandPools[i] = pool;
			DisposalQueue.EnqueueInGlobal(() => Context.Vk.DestroyCommandPool(Context.Device, pool, null));
		}

		_vertexShader = CreateShader("./assets/shaders/ui/rectangle.vert", ShaderKind.VertexShader);
		_vertexShader.EnqueueGlobalDispose();

		_fragmentShader = CreateShader("./assets/shaders/ui/rectangle.frag", ShaderKind.FragmentShader);
	...
	private static CommandBuffer CreateCommandBuffer(int imageIndex)
	{
		...
		Check(Context.Vk.AllocateCommandBuffers(Context.Device, allocInfo, out var cmd), "Failed to allocate ui command buffer.");
	...
	private static void CreateGlobalDataDescriptors()
	{
		...
		Check(Context.Vk.CreateDescriptorSetLayout(Context.Device, &globalDataLayoutCreateInfo, null, out _globalDataLayout),
		...
		Check(Context.Vk.CreateDescriptorPool(Context.Device, &globalDataPoolCreateInfo, null, out _globalDataPool),
		...
		Check(Context.Vk.AllocateDescriptorSets(Context.Device, &globalAllocInfo, out _globalDataSet),
	...
	private static void CreateDescriptorSetLayouts()
	{
		...
		Check(Context.Vk.CreateDescriptorSetLayout(Context.Device, &texturesCreateInfo, null, out _texturesLayout),
		...
		Check(Context.Vk.CreateDescriptorSetLayout(Context.Device, &componentDataCreateInfo, null, out _componentDataLayout),
			"Failed to create ui data descriptor set layout.");
		...
		Check(Context.Vk.CreateDescriptorSetLayout(Context.Device, &vertMaterialDataCreateInfo, null, out _vertMaterialDataLayout),
		...
		Check(Context.Vk.CreateDescriptorSetLayout(Context.Device, &fragMaterialDataCreateInfo, null, out _fragMaterialDataLayout),
	...
	private static void CreateDescriptorPools()
	{
		...
		Check(Context.Vk.CreateDescriptorPool(Context.Device, &texturesCreateInfo, null, out _texturesPool),
		...
		Check(Context.Vk.CreateDescriptorPool(Context.Device, &componentDataCreateInfo, null, out _componentDataPool),
		...
		Check(Context.Vk.CreateDescriptorPool(Context.Device, &materialDataCreateInfo, null, out _materialDataPool),
	...
	private static void CreateDescriptorSets()
	{
		...
		Check(Context.Vk.AllocateDescriptorSets(Context.Device, &texturesAllocInfo, out _texturesSet),
		...
		Check(Context.Vk.AllocateDescriptorSets(Context.Device, dataAllocInfo, out _componentDataSets[0]),
		...
		Check(Context.Vk.AllocateDescriptorSets(Context.Device, &vertMaterialDataAllocInfo, out _vertexMaterialDataSet),
			"Failed to allocate ui data descriptor sets.");
		...
		Check(Context.Vk.AllocateDescriptorSets(Context.Device, &fragMaterialDataAllocInfo, out _fragmentMaterialDataSet),
			"Failed to allocate ui data descriptor sets.");
		UpdateMaterialDataDescriptorSets();
	}
	...
	private static void CreatePipelines()
	{
		...
		Check(Context.Vk.CreateGraphicsPipelines(Context.Device, _pipelineCache, 2, createInfos[0].AsPointer(),
